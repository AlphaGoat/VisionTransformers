...
class TestCudaKernel(unittest.TestCase):
    def test_batch_matrix_mul(self):
    ...
    def test_batch_softmax_forward(self):
        ...
        test_tensor = torch.randn((2, 3, 4), dtype=torch.float32).cuda()
        output_tensor = batch_softmax_module.batch_softmax_forward(test_tensor)
        expected_output = torch.nn.functional.softmax(test_tensor, dim=-1)
        self.assertTrue(torch.allclose(output_tensor, expected_output, atol=1e-6))

    def test_batch_softmax_backward(self):
        batch_softmax_module = cpp_extension.load(
            name="batch_softmax_backward",
            sources=["./vision_transformers/torch-ext/torch_binding.cpp", "./vision_transformers/cuda/deformable_attn_kernel.cu"],
            extra_cflags=["-O3"],
            extra_ldflags=["-lcudart", "-lcuda"],
            extra_include_paths=cpp_extension.include_paths(device_type="cuda"),
        ...
        test_tensor = torch.randn((2, 3, 4), dtype=torch.float32).cuda()
        softmax_output = torch.nn.functional.softmax(test_tensor, dim=-1)
        grad_output = torch.randn((2, 3, 4), dtype=torch.float32).cuda()
        output_tensor = batch_softmax_module.batch_softmax_backward(grad_output, softmax_output)
        # Manually compute expected gradient
        expected_output = softmax_output * (grad_output - (grad_output * softmax_output).sum(dim=-1, keepdim=True))
        self.assertTrue(torch.allclose(output_tensor, expected_output, atol=1e-6))
